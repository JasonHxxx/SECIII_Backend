...
public class TaskController {
    ...
    public PageInfo<TaskVO> getHallTasks(@PathVariable Integer page) {
        return taskService.getHallTasks(page, Constant.PAGE_SIZE1);
    }
    ...
    public PageInfo<TaskVO> getAllTasks(@PathVariable Integer page) {
        return taskService.getAllTasks(page, Constant.PAGE_SIZE1);
    }

    @GetMapping("/recommendedTasks/{page}")
    @ApiOperation(value="众包工人：获取系统推荐的任务")
    @ApiImplicitParam(name = "page", value = "页码", paramType = "path", required = true)
    public PageInfo<TaskVO> getRecommendedTasks(@PathVariable Integer page, @RequestParam Integer uid) {
        return taskService.getRecommendedTasks(page,Constant.PAGE_SIZE1,uid);
    }
